# System Map: HazAT original vs maintained gpmarques fork

**Path:** [Interactive subagents overview](interactive-subagents-overview.md) › HazAT comparison
**Question:** What materially differs between `HazAT/pi-interactive-subagents` and the maintained `gpmarques/pi-interactive-subagents` fork at the fixed revisions?
**Boundary:** HazAT `main` at exact commit `c100577` (`v3.7.2`) versus the fork at exact commit `b403b02484aa545b72a0a852aee9ecce524fa6f8` (package version remains `3.7.2`). This documentation-only audit does not move either implementation endpoint. The comparison covers public tools, terminal backends, lifecycle behavior, sandboxing, bundled roles, workflows, repository lineage, and verification; linked Pi, provider, and terminal internals are outside it.
**Horizon:** Behavior at these two fixed revisions, not a forecast of either project's roadmap.
**Evidence basis:** Static source/README comparison at both commits; HazAT runtime suites were not executed. Fresh no-provider/no-live fork verification against `b403b02` passed unit 204/204, top-level hermetic/offline 38/38, and bundled-profile offline capability 1/1 on Pi 0.84.4. Historical fork evidence recorded at this boundary includes forced live no-model surfaces (Herdr 8/8, tmux 7/7), Stage 5 model-backed Herdr (2/2, 15 Sol responses, 0 provider errors), Stage 6 provider-backed Herdr (6/6, 40 Sol responses, 0 errors/timeouts), and a live visual-browser observation. Those surfaces, provider calls, and browser checks were not rerun in this audit.
...
- **This fork at `b403b02`:** supports exactly tmux and Herdr through `mux.ts` plus private `mux-adapters.ts`; exposes five by-name lifecycle tools, fixed-profile nesting, parked questions, destructive kill, validated sandbox replay, researcher-only web capabilities, and a narrower workflow layer.

The fork did not simply select one HazAT backend and did not merge HazAT wholesale. It first ported the source into a separate-root history, then redesigned lifecycle and capability boundaries. The `b403b02` snapshot has a backend-neutral seam for tmux/Herdr and selectively adds Herdr; it does **not** claim HazAT's cmux, zellij, or WezTerm support.
...
| Pi extension/tool containment | Tool list can narrow calls, while ambient extension discovery remains active | `--no-extensions`, explicit tools and `-e` backing-extension paths, plus nested-role allowlist; cwd/agent-dir ambient extensions are not inherited |
...
| Built-in commands | `/plan`, `/iterate`, `/subagent` | `/subagent` only; it queues a parent-model instruction to invoke the tool rather than spawning directly |
...
Historical forced live no-model tests passed 8/8 on Herdr and 7/7 on tmux; they were not rerun in this audit. The cases exercised focus-preserving creation and cleanup of tracked resources, but did not assert equality of complete pre/post workspace surface sets. These observations establish the tested host paths, not every terminal version or shell. They are distinct from the historical model-backed tiers: the bounded Stage 5 smoke covered completion/resume and kill/forget, while the Stage 6 broad provider-backed suite covered six broader current-contract lifecycle cases. Both model-backed tiers were forced-Herdr runs; neither supplies model-backed tmux or HazAT runtime evidence.
...
The fork requires the parent to have a persistent Pi session file—`--no-session` parents cannot spawn—and centralizes initial Pi launch and both completed-name entrypoints around one sandbox representation:

- omitted Pi profile tools yield only `ask_question`, not ambient Pi defaults;
- restricted children receive `--no-extensions`, an explicit tool list, and exact `-e` backing-extension paths; cwd and effective agent-directory selection do not grant ambient extensions;
- `safe_bash` is a separately loaded fixed-pattern wrapper, not protection for built-in `bash`, and any ambient `bash-guard` remains disabled;
- only bundled `researcher` declares all four canonical `pi-web-access` tools (`web_search`, `fetch_content`, `get_search_content`, `source_check`), along with `safe_bash` rather than `bash`;
...
The compared histories have separate roots and no Git merge base. The fork history records a source port (`c47a653`) followed by redesign, including a stage that supported only tmux; the fixed `b403b02` endpoint includes Herdr behind a backend-neutral seam.
...
- **Provider lifecycle comparison:** Historical Stage 6 evidence covers six broad current-contract fork paths, including questions and parallel work, while historical Stage 5 evidence separately covers explicit resume and kill/forget. HazAT still has no runtime evidence here, and neither fork suite covers deep nesting, every race, or every provider path. **Needed:** create equivalent bounded HazAT scenarios where the contracts align and add fork cases only for material gaps.
...
**Parent update:** This fork supports tmux/Herdr through a backend-neutral seam, while HazAT supports cmux/tmux/zellij/WezTerm. The fork still diverges selectively rather than merging HazAT wholesale. Historical Stage 5 evidence records public completion/resume and kill/forget on model-backed Herdr for two bounded scenarios; historical Stage 6 evidence separately records six broad fixed-profile lifecycle cases on provider-backed Herdr, alongside freshly reverified fail-closed sandbox and capability boundaries.

...
**Revision boundary:** Fork implementation at exact commit `b403b02484aa545b72a0a852aee9ecce524fa6f8` (package version remains `3.7.2`). This documentation-only audit does not move that implementation endpoint. The boundary includes the extension entry point, mux surface layer, child lifecycle helper, session/artifact state, bundled agents, and tests; Pi internals, provider behavior, and external terminal implementations remain outside it.
**Horizon:** One persistent parent session selecting a backend, spawning, supervising, messaging, killing, completing, and later resuming children.
**Evidence basis:** Static inspection at the fixed commit plus fresh no-provider/no-live verification for this documentation audit: unit 204/204; top-level hermetic/offline tests 38/38 (lifecycle 20/20, mux 14/14, visual profile 2/2, system-prompt 1/1 with 21 internal assertions, production web-preflight boundary 1/1); and one six-runtime bundled-profile capability test, 1/1. The capability checks used installed Pi 0.84.4 in offline RPC mode, invoked no model or web tool, and proved only `researcher` receives all four canonical web tools. Herdr/tmux surfaces, provider-backed runs, and the visual-browser observation recorded below are historical evidence incorporated at this boundary; none was rerun in this audit.
...
It also has `/subagent <agent> <task>`, which sends the parent model a user instruction to call `subagent`; it does not bypass the model/tool turn or spawn directly. A parent without a persistent session file cannot spawn. Pi children may receive `ask_question`.
